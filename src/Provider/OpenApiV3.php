...
class OpenApiV3
{
    ...
    {
        $spec = $this->spec;
        $this->version = $this->getVersion($spec);

        foreach ($this->getSchemas($spec) as $className => $schema) {

            $definition = $this->getDefinition($className) ?? $this->createDefinition($className);

            foreach ($schema['properties'] as $propertyName => $propertySchema) {

                $readOnly = $propertySchema['readOnly'] ?? null;
                $writeOnly = $propertySchema['writeOnly'] ?? null;

                $visibility = 'public';

                $propertyDefinition = $definition->addProperty($propertyName, $visibility);
                ...
                $this->addDefaultValue($propertyDefinition, $propertySchema);
                $this->addDeprecatedAnnotation($propertyDefinition, $propertySchema);
                ...
                if ((bool) $readOnly === true) {
                    $propertyDefinition->addComment("@Symfony\Component\Serializer\Annotation\Groups({'response'})");
                }

                if ((bool) $writeOnly === true) {
                    $propertyDefinition->addComment("@Symfony\Component\Serializer\Annotation\Groups({'request'})");
                }

                $this->addExampleAnnotation($propertySchema, $propertyDefinition);
...
//            }
    ...
    {
        $choices = implode(',', array_map(static function ($value) {
    ...
    private function addDefaultValue(PropertyDefinition $propertyDefinition, $propertySchema): void
    {
        if (array_key_exists('default', $propertySchema)) {
            $propertyDefinition->setValue($propertySchema['default']);
        }
    }

    private function addDeprecatedAnnotation(PropertyDefinition $propertyDefinition, $propertySchema): void
    {
        if (array_key_exists('deprecated', $propertySchema)) {
            $propertyDefinition->addComment("@deprecated\n");
        }
    }
    ...
    private function createDefinition(string $className, bool $interface = false): ClassDefinition
    {
        ...
        $definition->addComment("This is autogenerated object.\n");
    ...
    private function getSchemas(array $spec): array
    {
        return $spec['components']['schemas'] ?? [];
    }

    private function getVersion(array $spec): ?string
    {
        return $spec['info']['version'] ?? null;
    }

    private function addExampleAnnotation(array $propertySchema, PropertyDefinition $propertyDefinition): void
    {
        if(array_key_exists('example', $propertySchema)) {
            $propertyDefinition->addComment(sprintf("\n@example %s\n", $propertySchema['example']));
        }
    }
}